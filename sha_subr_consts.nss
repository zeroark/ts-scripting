...
const string SUBRACE_ENGINE_VERSION = "Starry Night 3.0.6.9.13";
...
// :: To disable, use ""
const string SUBRACE_SPELLHOOKS = "sha_spellhooks";
...
// :: True: Destroy duplicate subrace objects found in inventory,
// :: False: Just ignore them.
// :: Default Value: TRUE
const int SEARCH_AND_DESTROY_UNIQUE_OBJECTS = TRUE;
...
const int SSE_TREAT_ALIAS_AS_SUBRACE = 1;
...
// b13
    // if set to true PC can't fly inside
    const int SSE_LIMIT_FLIGHT_INTERIOR = TRUE;

    // if set to true PC can't fly underground
    const int SSE_LIMIT_FLIGHT_UNDERGROUND = FALSE;

    //true to teleport to faction start,
    //false for SHA_ENTER, Subrace helper, etc to enter mod
    const int USE_SSE_SUBRACE_START = FALSE;

    // increase if needed for busy PW servers, reduce for smaller mods
    const float SSE_LOAD_FACTOR = 0.1;
...
// Apply default subraces to PCs with blank subraces as per DMG 3.5
    ...
    const string SUBRACE_DWARF_DEFAULT     = "Dwarf-Hill";
    const string SUBRACE_ELF_DEFAULT       = "Elf-High";
    const string SUBRACE_GNOME_DEFAULT     = "Gnome-Rock";
    const string SUBRACE_HALFELF_DEFAULT   = "Half-Elf-High";
    const string SUBRACE_HALFLING_DEFAULT  = "Halfling-Lightfoot";
    const string SUBRACE_HALFORC_DEFAULT   = "Half-Orc-Savage";
    const string SUBRACE_HUMAN_DEFAULT     = "Human";
//3.0.6.4
    const string SUBRACE_DEATH_DEFAULT  = "wp_sha_death";
// 3.0.6.2
...
const float XP_SLIDER_VALUE = 10.0;
...
const string LETO_PORTAL_SERVER_PASSWORD = "";
...
// 3.0.6.9.11 renamed
const int SSE_EYE_COLOR_CYAN = 1;
const int SSE_EYE_COLOR_GREEN = 2;
const int SSE_EYE_COLOR_ORANGE = 3;
const int SSE_EYE_COLOR_PURPLE = 4;
const int SSE_EYE_COLOR_RED = 5;
const int SSE_EYE_COLOR_WHITE = 6;
const int SSE_EYE_COLOR_YELLOW = 7;
