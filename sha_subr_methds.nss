...
void EquipTemporarySubraceSkin(string SubraceStorage, object oPC, int iTime)
{
   ...
   if(ResrefExistingSkin != sSkin)
   {
       ...
       if(GetIsObjectValid(ExistingSkin))
       {
           ...
           DestroyObject(ExistingSkin, 0.2*SSE_LOAD_FACTOR);
       }
       SetIdentified(NewSkin, TRUE);
       DelayCommand(1.0*SSE_LOAD_FACTOR, AssignCommand(oPC, SHA_SubraceForceEquipItem(NewSkin, INVENTORY_SLOT_CARMOUR)));
   }
}
...
void EquipTemporarySubraceClaw(string SubraceStorage, object oPC, int iTime)
{
   ...
   if((ResrefExistingRClaw != sRightClaw) || (ResrefExistingLClaw != sLeftClaw))
   {
         ...
         DelayCommand(9.0*SSE_LOAD_FACTOR, SSE_MessageHandler(oPC,MESSAGE_SUBRACE_CLAWS_SUCCESSFULLY_EQUIPPED));
    }
    if(ResrefExistingRClaw != sRightClaw)
    {
           ...
           if(sRightClaw == "none" || GetIsObjectValid(ExistingRightClaw))
           {
               ...
               DestroyObject(ExistingRightClaw, 0.2*SSE_LOAD_FACTOR);
           }
           SetIdentified(NewRClaw, TRUE);
           DelayCommand(1.0*SSE_LOAD_FACTOR, AssignCommand(oPC, SHA_SubraceForceEquipItem(NewRClaw, INVENTORY_SLOT_CWEAPON_R)));
     }
     if(ResrefExistingLClaw != sLeftClaw)
     {
           ...
           if(sLeftClaw == "none"  || GetIsObjectValid(ExistingLeftClaw))
           {
               ...
               DestroyObject(ExistingLeftClaw, 0.3*SSE_LOAD_FACTOR);
           }
           SetIdentified(NewLClaw, TRUE);
           DelayCommand(5.0*SSE_LOAD_FACTOR, AssignCommand(oPC, SHA_SubraceForceEquipItem(NewLClaw, INVENTORY_SLOT_CWEAPON_L)));
     }
}
    ...
    while(i <= SubraceUniqueItemCount)
    {
       ...
       if(resref != "")
       {
           ...
           if(iLvl == iLevel)
           {
              ...
              DelayCommand(1.0*SSE_LOAD_FACTOR, SSE_MessageHandler(oPC, MESSAGE_SUBRACE_ACQUIRED_UNIQUE_ITEM, GetName(oItem)));
              SetSubraceDBInt(SUBRACE_DATABASE, SubraceStorage + "_" + SUBRACE_UNIQUEITEM, iLevel, oPC);
              SetIdentified(oItem, TRUE);
              SetPlotFlag(oItem, TRUE);
              SetItemCursedFlag(oItem, TRUE);
//b13 added S&D Subrace Objects
                if (SEARCH_AND_DESTROY_UNIQUE_OBJECTS)
                {
                    object oCheck = GetFirstItemInInventory(oPC);
                    string sCheck = GetTag(oItem);
                    while (oCheck != OBJECT_INVALID)
                    {
                        if ((oCheck!=oItem)&&(sCheck == GetTag(oCheck)))
                        {
...
void LoadSubraceFromScript(string Script, int Conditions=SSE_SUBRACE_LOADER_CONDITION_ALWAYS_LOAD)
{
    ...
    if(Load)
    {
        ...
        DelayCommand((Timer/100.0)*SSE_LOAD_FACTOR, ExecuteScript(Script, oStorer));
    }
}
...
void SSE_ModuleLoadEvent(int doNWNXInit=FALSE)
{
    ...
    if(!Scripts && !Subraces && Delays<4)
    {
        ...
        DelayCommand(10.0*SSE_LOAD_FACTOR, SSE_ModuleLoadEvent());
    }
    ...
    {
        Delays|=0x80000000;
        DelayCommand(5.0*SSE_LOAD_FACTOR, SSE_ModuleLoadEvent());
        SetSSEInt(SSE_SUBRACE_LOADER_AMOUNT_OF_DELAYS, Delays);
    }
...
void ApplySubrace(object oTarget, string subrace)
{
    ...
    if(ENABLE_LETO)
    {
        ...
        DelayCommand(4.0*SSE_LOAD_FACTOR, SetLocalString(oTarget, "SUBR_FileName", GetBicFileName(oTarget)));
    }
    DeleteSubraceInfoOnPC(oTarget);
    int ID = GetSubraceID(subrace);
    DelayCommand(1.5*SSE_LOAD_FACTOR, LoadSubraceInfoOnPC(oTarget, subrace));
    DelayCommand(2.6*SSE_LOAD_FACTOR, SetSubraceDBInt(SUBRACE_DATABASE, SUBRACE_TAG + "_" + GetSubraceNameByAlias(subrace, TRUE), SUBRACE_ACCEPTED, oTarget));
    DelayCommand(4.0*SSE_LOAD_FACTOR, ApplyPermanentSubraceSpellResistance(ID, oTarget));
    DelayCommand(5.5*SSE_LOAD_FACTOR, ApplyPermanentSubraceAppearance(ID, oTarget));
// 3.0.6.6
    DelayCommand(6.0*SSE_LOAD_FACTOR, ApplySubraceColors(oTarget));
    DelayCommand(6.5*SSE_LOAD_FACTOR, ApplySubraceEyeColors(oTarget));

//3.0.6.9
    DelayCommand(7.0*SSE_LOAD_FACTOR, ApplySubraceHead(oTarget));

    DelayCommand(7.5*SSE_LOAD_FACTOR, CheckIfCanUseEquipedWeapon(oTarget));
    DelayCommand(11.5*SSE_LOAD_FACTOR,CheckIfCanUseEquippedArmor(oTarget));
}
...
//3.0.6.9 extra optimazation
    ...
    DelayCommand(3.0*SSE_LOAD_FACTOR, SetLocalInt(oPC, SUBRACE_INFO_LOADED_ON_PC, TRUE));
    DelayCommand(4.0*SSE_LOAD_FACTOR, ApplyTemporarySubraceAppearance(SubraceStorage, oPC, iTime));
    DelayCommand(4.5*SSE_LOAD_FACTOR, ChangeMiscellaneousSubraceStuff(oPC, GetPlayerLevel(oPC)));
    DelayCommand(5.5*SSE_LOAD_FACTOR, ApplySubraceEffect(oPC, SubraceStorage, iTime));
    DelayCommand(6.5*SSE_LOAD_FACTOR, ApplyPermanentSubraceSpellResistance(ID, oPC));
    DelayCommand(7.5*SSE_LOAD_FACTOR, ApplyPermanentSubraceAppearance(ID, oPC));

//3.0.6.6
    DelayCommand(8.0*SSE_LOAD_FACTOR, ApplySubraceColors(oPC));
    DelayCommand(8.5*SSE_LOAD_FACTOR, ApplySubraceEyeColors(oPC));

//3.0.6.9
    DelayCommand(9.0*SSE_LOAD_FACTOR, ApplySubraceHead(oPC));

    DelayCommand(10.0*SSE_LOAD_FACTOR, EquipTemporarySubraceSkin(SubraceStorage, oPC, iTime));
    DelayCommand(13.0*SSE_LOAD_FACTOR, EquipTemporarySubraceClaw(SubraceStorage, oPC, iTime));
    DelayCommand(14.0*SSE_LOAD_FACTOR, SSE_MessageHandler(oPC, MESSAGE_SUBRACE_DATA_LOADED));
    DelayCommand(20.0*SSE_LOAD_FACTOR, ChangeSubraceFactions(oPC, subrace));
    ...
    if(NeedsToRelog)
    {
       ...
       {
           DelayCommand(24.0*SSE_LOAD_FACTOR, PopUpDeathGUIPanel(oPC, FALSE, FALSE, 0, SUBRACE_ENGINE + SSE_GetStandardMessage(MESSAGE_LETO_PLEASE_RELOG)));
           if(LETO_AUTOMATICALLY_BOOT)
           {
               DelayCommand(24.2*SSE_LOAD_FACTOR, SSE_MessageHandler(oPC, MESSAGE_LETO_AUTOBOOT, IntToString(LETO_AUTOMATIC_BOOT_DELAY) ));
               DelayCommand((24.2*SSE_LOAD_FACTOR) + IntToFloat(LETO_AUTOMATIC_BOOT_DELAY), DelayBoot(oPC));
           }
       }
       else
       {
              ...
              {
                  DelayCommand(24.2*SSE_LOAD_FACTOR, SSE_MessageHandler(oPC, MESSAGE_LETO_AUTOPORTAL, IntToString(LETO_AUTOMATIC_PORTAL_DELAY) ));
//b13 fix
                  DelayCommand(24.2*SSE_LOAD_FACTOR +
                  IntToFloat(LETO_AUTOMATIC_PORTAL_DELAY),
                  ActivatePortal(oPC, LETO_PORTAL_IP_ADDRESS,
                  LETO_PORTAL_SERVER_PASSWORD,
                  LETO_PORTAL_WAYPOINT, TRUE));
              }
              else
              {
                   ...
                   DelayCommand(24.2*SSE_LOAD_FACTOR, SSE_MessageHandler(oPC, MESSAGE_LETO_DONT_PANIC_JUSTPORTING));
//b13 fix
                   DelayCommand(24.2*SSE_LOAD_FACTOR + IntToFloat(LETO_AUTOMATIC_PORTAL_DELAY), ActivatePortal(oPC, LETO_PORTAL_IP_ADDRESS, LETO_PORTAL_SERVER_PASSWORD, sWPTag, TRUE));
                   DelayCommand(24.2*SSE_LOAD_FACTOR + IntToFloat(LETO_AUTOMATIC_PORTAL_DELAY), DestroyObject(oWaypoint, 0.1));
              }
       }
       DelayCommand(18.0*SSE_LOAD_FACTOR, SetSubraceDBInt(SUBRACE_DATABASE, SUBRACE_TAG + LETO_CHANGES_MADE_FOR_THIS_LEVEL + "_" + subraceBaseNameLower, Level, oPC));
    }
}
...
void DeleteSubraceInfoOnPC(object oPC, int ClearSubraceField=FALSE)
{
    ...
    DelayCommand(0.2*SSE_LOAD_FACTOR, DeleteSubraceInfoInDatabase(oPC, GetSubraceNameByAlias(subrace, TRUE)));
    ...
    DestroyObject(Skin, 0.1*SSE_LOAD_FACTOR);
    ...
    DestroyObject(ClawLeft, 0.5*SSE_LOAD_FACTOR);
    DestroyObject(ClawRight, 0.5*SSE_LOAD_FACTOR);
       ...
       if(resref != "")
       {
           ...
           if(iLevel <= GetPlayerLevel(oPC))
           {
               ...
               while(GetIsObjectValid(oItem))
               {
                   ...
                   {
                      SetPlotFlag(oItem, FALSE);
                      DestroyObject(oItem, 0.1*SSE_LOAD_FACTOR);
                   }
                  oItem = GetNextItemInInventory(oPC);
               }
           }
   ...
   if(ClearSubraceField)
    {
        ...
        {
            //3.0.6.5
            DelayCommand(0.5*SSE_LOAD_FACTOR, SetSubRace(oPC, ""));
            if (USE_SSE_DEFAULT_RACES)
            {
                DelayCommand(0.6*SSE_LOAD_FACTOR, CheckAndApplyDefaultSubrace(oPC));
            }
        }
    }
   DelayCommand(1.0*SSE_LOAD_FACTOR, SSE_MessageHandler(oPC, MESSAGE_SUBRACE_PURGED));
...
void SHA_ApplyTemporaryStats(object oPC, string SubraceStorage, int iCurrentTime, int iTime, int AreasReq/*int AreaUndAbove, int AreaIntExt, int AreaNatArt*/)
{
    ...
    DelayCommand(1.0*SSE_LOAD_FACTOR, TemporaryStatsVisualFX(oPC));
    DelayCommand(1.0*SSE_LOAD_FACTOR, SSE_MessageHandler(oPC, MESSAGE_ABILITY_SCORES_CHANGED) );
   ...
   if(Restrictions)
    {
       if(SHA_TestItemReq(GetItemType(oItem), Restrictions))
       {
          ...
          DelayCommand(0.3*SSE_LOAD_FACTOR, AssignCommand(oPC, SHA_SubraceForceUnequipItem(oItem)));
       }
   }
...
void CheckIfCanUseEquipedWeapon(object oPC)
{
    ...
    DelayCommand(3.0*SSE_LOAD_FACTOR, CheckCanUseItem (Wep2, oPC, 1));
}
...
// 3.0.6.11 - moon: Now this case bug is fixed and stays fixed!
void InitiateSubraceChecking(object oPC) {
  // No lowercase on the alias.
  // we use it to print later in LoadSubraceInfoOnPC and it
  // is smart enough  to lowercase when needed!
  ...
  if(subrace == "")  {
    // no sub-race
    if (USE_SSE_DEFAULT_RACES) {
      // use default sub-race
      // Assume that there is no previously stored cased value.
      // Even if there is, it will be moved next time.
      // Of course it might have been a reject, but
      //  if there is a restriction on default sub-races
      // then how are they a default?
      CheckAndApplyDefaultSubrace(oPC);
      subraceAlias = GetSubRace(oPC);
      subrace = GetSubraceNameByAlias(subraceAlias);
    } else {
      // no default sub-race means nothing to do.
      return;
    }
  } else if(subrace == subraceCased){
    // The sub-race name is already lower-case.
    // empty subraceCased to save ourselves an extra call to the DB.
    subraceCased = "";
  }

  if(subrace != "") {
    //3.0.6.9 fix + 3.0.6.11 refix
    int check1 = SUBRACE_UNINITIALIZED;
    if(subraceCased != "") {
      // We got two different casing... try the mixed case first.
      check1 = GetSubraceDBInt(SUBRACE_DATABASE, SUBRACE_TAG + "_" + subraceCased, oPC);
      if(check1 != SUBRACE_UNINITIALIZED) {
   // There was a value with mixed-case, move it to the lowercase location.
   // and remove the mixed-case entry.
   ...
   DeleteSubraceDBInt(SUBRACE_DATABASE, SUBRACE_TAG + "_" + subraceCased, oPC);
    ...
    if(check1 == SUBRACE_UNINITIALIZED){
      // This happens if A) subraceCased was all lowercase or
      // B) There was no value stored in the database for subraceCased.
      check1 = GetSubraceDBInt(SUBRACE_DATABASE, SUBRACE_TAG + "_" + subrace, oPC);
    }
    if(check1 == SUBRACE_REJECTED){
      //Rejected so stop here.
      return;
    }
    if(check1 == SUBRACE_UNINITIALIZED) {
   ...
   return;
      ...
      // Since there is return in the two previous branches, we do not need else if here.
      SetSubraceDBInt(SUBRACE_DATABASE, SUBRACE_TAG + "_" + subrace, SUBRACE_ACCEPTED, oPC);
    }

    // At this point - sub-race is accepted and the database is updated if needed!
    if(SEARCH_AND_DESTROY_SKINS_IN_INVENTORY) {
      SearchAndDestroySkinsAndClaws(oPC);
    }
    DelayCommand(1.0*SSE_LOAD_FACTOR, LoadSubraceInfoOnPC(oPC, subraceAlias));
//b13
    if (USE_SSE_SUBRACE_START)
    {
        DelayCommand(22.0*SSE_LOAD_FACTOR, Subrace_MoveToStartLocation(oPC, subraceAlias));
    }
//end b13
    }
}
...
// 3.0.6.5
  ...
  {
     DelayCommand(0.5*SSE_LOAD_FACTOR, AssignCommand(oPC, JumpToLocation(GetLocation(oWP))));
     DelayCommand(0.5*SSE_LOAD_FACTOR, SSE_MessageHandler(oPC, MESSAGE_SUBRACE_MOVE_TO_START_LOCATION, CapitalizeString(subrace)) );
  }
}
...
//3.0.6.5 fix - defaults to GetStartingLocation()
     if( GetIsObjectValid(oWP) )
     {
        DelayCommand(0.5*SSE_LOAD_FACTOR, AssignCommand(oPC, JumpToLocation(GetLocation(oDefaultWP))));
     }
     else
     {
        DelayCommand(0.5*SSE_LOAD_FACTOR, AssignCommand(oPC, JumpToLocation(GetStartingLocation())));
     }
  }
...
void ChangeSubraceFactions(object oPC, string subrace)
{
   ...
   if(Count)
   {
        ...
        while(Count != 0)
        {
            ...
            DelayCommand(fDelay*SSE_LOAD_FACTOR, Subrace_FactionAdjustment(oPC, FactionCreatureTag, Reputation));
            Count--;
        }
       ...
       while(switchSubraceNames != "")
       {
          ...
          //only check restrictions if need be.
           ...
           if(!check)
           {
               ...
               DelayCommand(4.2*SSE_LOAD_FACTOR, SSE_MessageHandler(oPC, MESSAGE_SUBRACE_SWITCHING, NewSubrace + "..."));
               DelayCommand(4.3*SSE_LOAD_FACTOR, SetSubRace(oPC, NewSubrace));
               DelayCommand(5.1*SSE_LOAD_FACTOR, LoadSubraceInfoOnPC(oPC, NewSubrace));
               DelayCommand(15.0*SSE_LOAD_FACTOR, CheckIfCanUseEquipedWeapon(oPC));
               DelayCommand(18.5*SSE_LOAD_FACTOR, CheckIfCanUseEquippedArmor(oPC));
               DelayCommand(21.5*SSE_LOAD_FACTOR, SSE_MessageHandler(oPC, MESSAGE_SUBRACE_SWITCHED));
               return;
           }
       }
   }
...
void SubraceOnPlayerLevelUp()
{
    ...
    int Level = GetPlayerLevel(oPC);
    ReapplySubraceAbilities(oPC);
    DelayCommand(2.5*SSE_LOAD_FACTOR, ChangeMiscellaneousSubraceStuff(oPC, Level));

    DelayCommand(2.0*SSE_LOAD_FACTOR, CheckAndGiveSubraceItems(oPC));
    int NeedsToRelog = CheckForLetoReLog(oPC, Level);
    if(NeedsToRelog)
    {
       if(!LETO_ACTIVATE_PORTAL)
       {
           DelayCommand(5.0*SSE_LOAD_FACTOR, PopUpDeathGUIPanel(oPC, FALSE, FALSE, 0, SUBRACE_ENGINE + SSE_GetStandardMessage(MESSAGE_LETO_PLEASE_RELOG)));
           if(LETO_AUTOMATICALLY_BOOT)
           {
               DelayCommand(5.2*SSE_LOAD_FACTOR, SSE_MessageHandler(oPC, MESSAGE_LETO_AUTOBOOT, IntToString(LETO_AUTOMATIC_BOOT_DELAY)));
               DelayCommand((5.2*SSE_LOAD_FACTOR) + IntToFloat(LETO_AUTOMATIC_BOOT_DELAY), DelayBoot(oPC));
           }
       }
       else
       {
              if(!LETO_PORTAL_KEEP_CHARACTER_IN_THE_SAME_PLACE)
              {
                  DelayCommand(5.2*SSE_LOAD_FACTOR, SSE_MessageHandler(oPC, MESSAGE_LETO_AUTOPORTAL, IntToString(LETO_AUTOMATIC_PORTAL_DELAY)));
//b13 fix
                  DelayCommand(5.2*SSE_LOAD_FACTOR + IntToFloat(LETO_AUTOMATIC_PORTAL_DELAY), ActivatePortal(oPC, LETO_PORTAL_IP_ADDRESS, LETO_PORTAL_SERVER_PASSWORD, LETO_PORTAL_WAYPOINT, TRUE));
              }
              else
              {
                   int RandomNumber = d100(1);
                   string sWPTag = "WP_SUBRACE_P" + IntToString(RandomNumber);
                   object oWaypoint = CreateObject(OBJECT_TYPE_WAYPOINT, "nw_waypoint001", GetLocation(oPC), FALSE, sWPTag);
                   DelayCommand(5.2*SSE_LOAD_FACTOR, SSE_MessageHandler(oPC, MESSAGE_LETO_DONT_PANIC_JUSTPORTING));
//b13 fix
                   DelayCommand(5.2*SSE_LOAD_FACTOR + IntToFloat(LETO_AUTOMATIC_PORTAL_DELAY), ActivatePortal(oPC, LETO_PORTAL_IP_ADDRESS, LETO_PORTAL_SERVER_PASSWORD, sWPTag, TRUE));
                   DelayCommand(15.0*SSE_LOAD_FACTOR + IntToFloat(LETO_AUTOMATIC_PORTAL_DELAY), DestroyObject(oWaypoint, 0.1));
              }
       }

       DelayCommand(5.0*SSE_LOAD_FACTOR, SetSubraceDBInt(SUBRACE_DATABASE, SUBRACE_TAG + LETO_CHANGES_MADE_FOR_THIS_LEVEL + "_" + GetStringLowerCase(GetSubRace(oPC)), Level, oPC));
    }
    DelayCommand(5.0*SSE_LOAD_FACTOR, CheckAndSwitchSubrace(oPC));
}
...
void ReapplySubraceAbilities(object oPC)
{
     ...
     ClearSubraceEffects(oPC);
     int iTime = SHA_GetCurrentTime();
     DelayCommand(1.5*SSE_LOAD_FACTOR, ApplyTemporarySubraceAppearance(SubraceStorage, oPC, iTime ));
     DelayCommand(2.0*SSE_LOAD_FACTOR, ApplyPermanentSubraceAppearance(ID, oPC));
// 3.0.6.6
     DelayCommand(2.5*SSE_LOAD_FACTOR, ApplySubraceColors(oPC));
     DelayCommand(3.0*SSE_LOAD_FACTOR, ApplySubraceEyeColors(oPC));

// 3.0.6.9
     DelayCommand(3.5*SSE_LOAD_FACTOR, ApplySubraceHead(oPC));

     DelayCommand(4.0*SSE_LOAD_FACTOR, SearchAndDestroySkinsAndClaws(oPC));
     DelayCommand(4.5*SSE_LOAD_FACTOR, ApplySubraceEffect(oPC, SubraceStorage, iTime));
     DelayCommand(5.0*SSE_LOAD_FACTOR, EquipTemporarySubraceSkin(SubraceStorage, oPC, iTime));
     DelayCommand(6.0*SSE_LOAD_FACTOR, EquipTemporarySubraceClaw(SubraceStorage, oPC, iTime));
     DelayCommand(6.1*SSE_LOAD_FACTOR, SetLocalInt(oPC, SUBRACE_INFO_LOADED_ON_PC, TRUE));
}
...
// b13 changed
    else if(sTag == "fly_to")
    {
        ...
        object oArea = GetArea(oPC);
        if (SSE_LIMIT_FLIGHT_INTERIOR && GetIsAreaInterior(oArea))
        {
            SendMessageToPC(oPC,"Flight limit: interior.");
            return;
        }
        else if (SSE_LIMIT_FLIGHT_UNDERGROUND && (!GetIsAreaAboveGround(oArea)))
        {
            SendMessageToPC(oPC,"Flight limit: underground.");
            return;
        }
        else
        {
...
//3.0.6.5
   ...
   if(!GetIsObjectValid(oArea))
   {
        ...
        DelayCommand(2.0*SSE_LOAD_FACTOR, SubraceOnClientEnter(oPC));
        return;
   }
   ...
   if(ENABLE_LETO && !infoLoaded)
   {
       ...
       DelayCommand(5.0*SSE_LOAD_FACTOR, SetLocalString(oPC, "SUBR_FileName", GetBicFileName(oPC)));
   }
   if(GetIsSSEDisabled())
   {
        SSE_MessageHandler(oPC, MESSAGE_SUBRACE_SSE_IS_SHUTDOWN);
   ...
   if(!infoLoaded)
   {
      DelayCommand(1.0*SSE_LOAD_FACTOR, InitiateSubraceChecking(oPC));
   }
   else if(infoLoaded && RELOAD_SUBRACE_INFORMATION_UPON_RELOGIN)
   {
       DeleteLocalInt(oPC, SUBRACE_INFO_LOADED_ON_PC);
       DelayCommand(1.0*SSE_LOAD_FACTOR, LoadSubraceInfoOnPC(oPC, subrace));
   }
   else
   {
      DelayCommand(1.0*SSE_LOAD_FACTOR, ReapplySubraceAbilities(oPC));
   }
}
